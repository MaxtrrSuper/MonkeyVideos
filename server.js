...
app.post('/api/videos', (req, res) => {
    ...
    db.run('INSERT INTO videos (uuid, url, user, description) VALUES (?, ?, ?, ?)',
        [uuidv4(), url, user, description || ''],
        function() { res.json({ success: true, data: { id: this.lastID } }); }
    );
});
...
app.post('/api/videos/upload', upload.single('video'), (req, res) => {
    ...
    db.run('INSERT INTO videos (uuid, url, title, user, description, is_uploaded) VALUES (?, ?, ?, ?, ?, 1)',
        [uuidv4(), url, title || '', user || 'anonymous', description || ''],
        function() { res.json({ success: true, data: { id: this.lastID, url } }); }
    );
});

app.get('/api/videos/search', (req, res) => {
    const q = req.query.q || '';
    db.all('SELECT * FROM videos WHERE user LIKE ? OR description LIKE ? LIMIT 20',
        [`%${q}%`, `%${q}%`], (err, rows) => res.json({ success: true, data: rows || [] }));
});

app.get('/api/videos/my', (req, res) => {
    db.all('SELECT * FROM videos WHERE user = ? ORDER BY created_at DESC',
        [req.query.user || ''], (err, rows) => res.json({ success: true, data: rows || [] }));
});
...
app.post('/api/videos/:id/like', (req, res) => {
    ...
    db.get('SELECT * FROM likes WHERE video_id = ? AND user_id = ?', [id, userId], (err, row) => {
        ...
        db.get('SELECT COUNT(*) as count FROM likes WHERE video_id = ?', [id], (err, r) => {
            res.json({ success: true, data: { liked: !row, likes: r.count } });
        });
    });
});
...
app.get('/api/videos/:id/comments', (req, res) => {
    db.all('SELECT * FROM comments WHERE video_id = ? ORDER BY created_at DESC',
        [req.params.id], (err, rows) => res.json({ success: true, data: rows || [] }));
});

app.post('/api/videos/:id/comments', (req, res) => {
    db.run('INSERT INTO comments (video_id, user, text) VALUES (?, ?, ?)',
        [req.params.id, req.body.user, req.body.text],
        function() { res.json({ success: true, data: { id: this.lastID } }); }
    );
});
...
app.get('/api/admin/stats', (req, res) => {
    if (req.query.token !== 'monkey-admin-secret-2024') return res.json({ success: false, error: 'Неверный токен' });
    db.get('SELECT COUNT(*) as count FROM videos', (err, v) => {
        db.get('SELECT COUNT(*) as count FROM users', (err, u) => {
            db.get('SELECT SUM(likes) as count FROM videos', (err, l) => {
                res.json({ success: true, data: { totalVideos: v, totalUsers: u, totalLikes: l } });
            });
        });
    });
});
...
app.post('/api/events/admin/create', (req, res) => {
    if (req.headers['admin-token'] !== 'monkey-admin-secret-2024') return res.json({ success: false, error: 'Неверный токен' });
    db.run('INSERT INTO events (title, description, emoji) VALUES (?, ?, ?)',
        [req.body.title, req.body.description || '', req.body.emoji || '🎉'],
        function() { res.json({ success: true, data: { id: this.lastID } }); }
    );
});
